...
(function (root) {
  ...
  const GUITAR_CHORDS = chordMap(String.raw`
'A/AB' => '-,0,2,1,2,0',
'A/B' => '0,0,2,4,2,0',
'A/B:1' => '-,0,7,6,0,0',
'A/D' => '-,0,0,2,2,0',
...
`);
  ...
  const KEY_CHORDS = chordMap(String.raw`
'MAJ' => '0,4,7',
'MIN' => '0,3,7',
'7' => '0,4,7,10',
'MIN7' => '0,3,7,10',
...
`);
  ...
  class MusicpadEngine {
    ...
    addTrack(trackSource) {
      ...
      let stringx = trackSource;
      stringx = stringx.replace(/[()]/g, '');
      stringx = stringx.replace(/x/gi, ' x ');
      stringx = stringx.replace(/' x/gi, "'x");
      stringx = stringx.replace(/, x/gi, ',x');
      stringx = stringx.replace(/syse x /gi, 'sysex');
      stringx = stringx.replace(/-/gi, ' - ');
      stringx = stringx.replace(/([A-GNTH]) - /gi, '$1-');
      stringx = stringx.replace(/, - /gi, ',-');
      stringx = stringx.replace(/\[ - /gi, '[-');
      stringx = stringx.replace(/g: - ,/gi, 'g:-,');
      stringx = stringx.replace(/, - ,/gi, ',-,');
      stringx = stringx.replace(/, - \]/gi, ',-]');
      stringx = stringx.replace(/=/gi, ' = ');
      stringx = stringx.replace(/\s+=/gi, '=');
      stringx = compactSpaces(stringx);

      const values = stringx.split(/\s+/);
